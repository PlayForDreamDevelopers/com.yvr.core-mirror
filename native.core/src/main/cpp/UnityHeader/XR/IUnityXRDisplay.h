...
// [InternalPackageValidation.PreventFilePublication]
...
typedef enum UnityXRRenderTextureFormat
{
    ...
    /// Don't create a whole new color texture; soft-alias the MSAA attachment, yet still construct the MSAA-resolved 1x texture as unique. This allows memory sharing for MSAA
...
typedef enum UnityXRShadingRateFormat
{
...
typedef struct UnityXRTextureData
{
...
typedef struct UnityXRRenderTextureDesc
{
    ...
    UnityXRShadingRateFormat shadingRateFormat;

    /// Data for shading rate texture.
    UnityXRTextureData shadingRate;
...
typedef struct UnityXRNextFrameDesc
{
    ...
    struct UnityXRRenderPass
    {
        ...
        /// A Unity motion-vector texture ID returned by CreateTexture
        UnityXRRenderTextureId motionVectorTextureId;
        ...
        // Platform data to be consumed by the engine code.
        void* foveatedRenderingInfo;
...
// Foveated Rendering configuration flags
typedef enum UnityXRFoveatedRenderingFlags
{
    /// No extra configuration.
    kUnityXRFoveatedRenderingFlagsNone = 0,

    /// Gaze can be used to setup foveated rendering.
    kUnityXRFoveatedRenderingFlagsGazeAllowed = 1 << 0,
} UnityXRFoveatedRenderingFlags;
...
typedef enum UnityXRFrameSetupHintsChanged
{
    ...
    /// The engine has changed the foveatedRenderingLevel state.  Check the #UnityXRFrameSetupHints::UnityXRAppSetup `foveatedRenderingLevel` field.
    kUnityXRFrameSetuphintsChangedFoveatedRenderingLevel = 1 << 10,

    /// The engine has changed the foveatedRenderingFlags state.  Check the #UnityXRFrameSetupHints::UnityXRAppSetup `foveatedRenderingFlags` field.
    kUnityXRFrameSetuphintsChangedFoveatedRenderingFlags = 1 << 11,
...
typedef struct UnityXRFrameSetupHints
{
    ...
    struct UnityXRAppSetup
    {
        ...
        /// App controls the intensity of the foveated rendering.
        float foveatedRenderingLevel;

        /// App configures optional settings for foveated rendering.
        UnityXRFoveatedRenderingFlags foveatedRenderingFlags;
...
typedef struct SUBSYSTEM_PROVIDER UnityXRDisplayGraphicsThreadProvider
{
    ...
    /// Update the head and controller poses, from the XR runtime, for use in late latching and rendering. Display subsystem uses the updated poses to generate new view/model matrices, and
    /// latches new poses at virtually the lowest possible latency, which is right before a flush to GPU.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
    /// @param[in] userData User specified userData that was filled out on #UnityXRDisplayGraphicsThreadProvider
    /// @param[out] lateLatchingPoses The newly updated pose from the XR runtime for the given frame, for which draw calls have been issued but not flushed.
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * UpdateLateLatchingRenderPoses)(UnitySubsystemHandle handle, void* userData, UnityXRPose lateLatchingPoses[kUnityXRLateLatchNodeCount]);

    /// Update the foveation offset state, from the XR runtime, for use in gaze foveated rendering. Display subsystem uses the updated NDC space foveation center to compute screen space foveation offset
    /// which is later used by a render pass. Currently, foveation offset is supported on Vulkan graphics device via fragment density map offset extension.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
    /// @param[in] userData User specified userData that was filled out on #UnityXRDisplayGraphicsThreadProvider
    /// @param[out] center The newly updated NDC space foveation center values from the XR runtime for the given frame.
    /// @param[out] renderPassIndex The render pass index corresponding to current frame render pass being rendered.
    /// @param[out] renderParamIndex The render param index corresponding to current view or slice of the currently rendered pass.
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * UpdateFoveationOffsetState)(UnitySubsystemHandle handle, void* userData, UnityXRVector2* center, int renderPassIndex, int renderParamIndex);
...
UNITY_REGISTER_INTERFACE_GUID(0xe0f8d3b3decc4ae3ULL, 0xabb9eaef54315fc4ULL, IUnityXRDisplayInterface); // 2022.2.0a4 - 4/6/2022
