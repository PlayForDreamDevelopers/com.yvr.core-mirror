...
/// The maximum size an event of type kInputFourCCXREventHapticSendBuffer can be.  The provider cannot declare UnityXRHapticCapabilities::bufferMaxSize greater than this, and the provider will not recieve a UnityXRHapticUpdate greater than this either.
...
/// A binary feature representing whether a device is being worn or actively used.
...
typedef enum UnityXRInputTrackingOriginModeFlags
{
    ...
    kUnityXRInputTrackingOriginModeDevice = 1 << 0,
    ...
    kUnityXRInputTrackingOriginModeFloor = 1 << 1,
    ...
    kUnityXRInputTrackingOriginModeTrackingReference = 1 << 2,
    /// Represents a moving tracking origin based on nearby spatial anchors.
    kUnityXRInputTrackingOriginModeUnbounded = 1 << 3
...
/// UnityXRInputProvider::HandleEvent's buffer parameter points to an object of this type when the event type is kInputFourCCXREventHapticGetCapabilities.
...
typedef struct UnityXRHapticCapabilities
{
    ...
    /// Tells Unity that this device supports the kInputFourCCXREventHapticSendImpulse event.
    bool supportsImpulse;
    /// Tells Unity that this device supports the kInputFourCCXREventHapticSendBuffer event.
    bool supportsBuffer;
