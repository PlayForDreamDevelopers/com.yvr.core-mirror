...
// Extension number: 516
#define XR_YVR_image_tracking 1
#define XR_YVR_image_tracking_SPEC_VERSION 1
#define XR_YVR_IMAGE_TRACKING_EXTENSION_NAME "XR_YVR_image_tracking"


XR_RESULT_ENUM(XR_ERROR_IMAGE_ID_INVALID_YVR, -1000516000);
XR_RESULT_ENUM(XR_ERROR_IMAGE_PATH_NOT_FOUND_YVR, -1000516001);
XR_RESULT_ENUM(XR_ERROR_IMAGE_TEMPLATE_NOT_EXIST_YVR, -1000516002);
XR_RESULT_ENUM(XR_ERROR_IMAGE_TEMPLATE_ALREADY_REGISTERED_YVR, -1000516003);

XR_STRUCT_ENUM(XR_TYPE_SYSTEM_IMAGE_TRACKING_PROPERTIES_YVR, 1000516000);
typedef struct XrSystemImageTrackingPropertiesYVR{
  XrStructureType               type;
  void*XR_MAY_ALIAS             next;
  XrBool32                      supportsImageTracking;
} XrSystemImageTrackingPropertiesYVR;

XR_STRUCT_ENUM(XR_TYPE_EVENT_DATA_IMAGE_TRACKING_UPDATE_YVR, 1000516001);
typedef struct XrEventDataImageTrackingUpdateYVR {
    XrStructureType             type;
    const void* XR_MAY_ALIAS    next;
    char*                       imageId;
    XrBool32                    isActive;
    ...
    XrExtent2Df                 size;
    XrTime                      time;
} XrEventDataImageTrackingUpdateYVR;

XR_STRUCT_ENUM(XR_TYPE_IMAGE_TEMPLATE_INFO_YVR, 1000516002);
typedef struct XrImageTemplateInfoYVR{
    XrStructureType             type;
    const void* XR_MAY_ALIAS    next;
    float                       physicalWidth;
    ...
    char*                       imageId;
    char*                       templatePath;
} XrImageTemplateInfoYVR;

XR_STRUCT_ENUM(XR_TYPE_IMAGE_SPACE_CREATE_INFO_YVR, 1000516003);
typedef struct XrImageSpaceCreateInfoYVR {
    XrStructureType             type;
    const void* XR_MAY_ALIAS    next;
    char*                       imageId;
    XrPosef                     poseInImageSpace;
} XrImageSpaceCreateInfoYVR;


typedef XrResult (XRAPI_PTR *PFN_xrSetImageTrackingYVR)(XrSession session, XrBool32 enabled);
typedef XrResult (XRAPI_PTR *PFN_xrRegisterImageTemplateYVR)(XrSession session, XrImageTemplateInfoYVR info);
typedef XrResult (XRAPI_PTR *PFN_xrUnRegisterImageTemplateYVR)(XrSession session, char* imageId);
typedef XrResult (XRAPI_PTR *PFN_xrCreateImageSpaceYVR)(XrSession session, const XrImageSpaceCreateInfoYVR* createInfo, XrSpace* outSpace);


#ifndef XR_NO_PROTOTYPES
#ifdef XR_EXTENSION_PROTOTYPES

XRAPI_ATTR XrResult XRAPI_CALL xrSetImageTrackingYVR(XrSession session, XrBool32 enabled);
XRAPI_ATTR XrResult XRAPI_CALL xrRegisterImageTemplateYVR(XrSession session, XrImageTemplateInfoYVR info);
XRAPI_ATTR XrResult XRAPI_CALL xrUnRegisterImageTemplateYVR(XrSession session, char* imageId);
XRAPI_ATTR XrResult XRAPI_CALL xrCreateImageSpaceYVR(XrSession session, const XrImageSpaceCreateInfoYVR* createInfo, XrSpace* outSpace);
#endif /* XR_EXTENSION_PROTOTYPES */
#endif /* !XR_NO_PROTOTYPES */

// Extension number: 517
#define XR_YVR_dp_composition 1
#define XR_YVR_dp_composition_SPEC_VERSION 1
#define XR_YVR_DP_COMPOSITION_EXTENSION_NAME "XR_YVR_dp_composition"

XR_STRUCT_ENUM(XR_TYPE_DP_COMPOSITION_PROPERTIES_YVR, 1000517001);
typedef struct XrDPCompostionPropertiesYVR{
  XrStructureType               type;
  void*XR_MAY_ALIAS             next;
  XrBool32                      supportsDPComposition;
} XrDPCompostionPropertiesYVR;

XR_STRUCT_ENUM(XR_TYPE_DP_COMPOSITION_LAYER_YVR, 1000517002);
// XrDPCompositionLayerYVR should link to XrCompositionLayerXXX's next
typedef struct XrDPCompositionLayerYVR
{
    XrStructureType             type; // XR_TYPE_DP_COMPOSITION_LAYER_YVR
    void *XR_MAY_ALIAS          next;
} XrDPCompositionLayerYVR;

XR_STRUCT_ENUM(XR_TYPE_DP_COMPOSITION_STATE_YVR, 1000517003);
typedef struct XrDPCompostionStateYVR {
    XrStructureType               type;
    void *XR_MAY_ALIAS            next;
    XrBool32                      connected; // DP source connect state
    int32_t                       width;     // DP input width
    int32_t                       height;    // DP input height
} XrDPCompostionStateYVR;

typedef XrResult (XRAPI_PTR *PFN_xrGetDPCompostionStateYVR)(XrSession session, XrDPCompostionStateYVR *state);
typedef XrResult (XRAPI_PTR *PFN_xrStartDPCompostionYVR)(XrSession session);
typedef XrResult (XRAPI_PTR *PFN_xrStopDPCompostionYVR)(XrSession session);

#ifndef XR_NO_PROTOTYPES
#ifdef XR_EXTENSION_PROTOTYPES
XRAPI_ATTR XrResult XRAPI_CALL xrGetDPCompostionStateYVR(XrSession session, XrDPCompostionStateYVR *state);
XRAPI_ATTR XrResult XRAPI_CALL xrStartDPCompostionYVR(XrSession session);
XRAPI_ATTR XrResult XRAPI_CALL xrStopDPCompostionYVR(XrSession session);
#endif /* XR_EXTENSION_PROTOTYPES */
#endif /* !XR_NO_PROTOTYPES */

// Extension number: 518
#define XR_YVR_persistent_passthrough_mesh 1
#define XR_YVR_persistent_passthrough_mesh_SPEC_VERSION 2
#define XR_YVR_PERSISTENT_PASSTHROUGH_MESH_EXTENSION_NAME "XR_YVR_persistent_passthrough_mesh"

XR_STRUCT_ENUM(XR_TYPE_PERSISTENT_PASSTHROUGH_MESH_MODE_INFO_YVR, 1000518000);
typedef struct XrPersistentPassthroughMeshModeInfo {
    XrStructureType type;
    void *XR_MAY_ALIAS next;
    XrBool32 followMode;
    XrControllerYVR controller;
} XrPersistentPassthroughMeshModeInfo;

XR_STRUCT_ENUM(XR_TYPE_PERSISTENT_PASSTHROUGH_MESH_TRANSFORM_YVR, 1000518001);
typedef struct XrPersistentPassthroughMeshTransformYVR {
    XrStructureType type;
    void *XR_MAY_ALIAS next;
    XrSpace space;
    XrTime time;
    XrPosef pose;
    XrVector3f scale;
} XrPersistentPassthroughMeshTransformYVR;

typedef XrResult(XRAPI_PTR *PFN_xrSetPersistentPassthroughMeshModeInfoYVR)(XrSession session, const XrPersistentPassthroughMeshModeInfo *setInfo);
typedef XrResult(XRAPI_PTR *PFN_xrGetPersistentPassthroughMeshModeInfoYVR)(XrSession session, XrPersistentPassthroughMeshModeInfo *getInfo);
typedef XrResult(XRAPI_PTR *PFN_xrSetPersistentPassthroughMeshYVR)(XrSession session, const XrSpaceTriangleMeshMETA *mesh);
typedef XrResult(XRAPI_PTR *PFN_xrSetPersistentPassthroughMeshTransformYVR)(XrSession session, const XrPersistentPassthroughMeshTransformYVR *transform);

#ifndef XR_NO_PROTOTYPES
#ifdef XR_EXTENSION_PROTOTYPES
XRAPI_ATTR XrResult XRAPI_CALL xrSetPersistentPassthroughMeshModeInfoYVR(XrSession session, const XrPersistentPassthroughMeshModeInfo *setInfo);
XRAPI_ATTR XrResult XRAPI_CALL xrGetPersistentPassthroughMeshModeInfoYVR(XrSession session, XrPersistentPassthroughMeshModeInfo *getInfo);
XRAPI_ATTR XrResult XRAPI_CALL xrSetPersistentPassthroughMeshYVR(XrSession session, const XrSpaceTriangleMeshMETA *mesh);
XRAPI_ATTR XrResult XRAPI_CALL xrSetPersistentPassthroughMeshTransformYVR(XrSession session, const XrPersistentPassthroughMeshTransformYVR *transform);
#endif /* XR_EXTENSION_PROTOTYPES */
#endif /* !XR_NO_PROTOTYPES */
// clang-format on
